...
exports.auth = async (req,res,next) => {
    ...
    //console.log(req.body.email)
        ...
        let station = await Police.findOne({$and:[{stationId:stationId},{password:password}]});
        if(!station){
            res.status(400).send({message:"Wrong username or Password!"});
            return;
        }
        const token = jwt.sign({stationId:station.stationId,id:station._id},
            "secret",{
                expiresIn:"11h"
            }
        )
        ...
        res.status(200).send({message:"Authentication Successful",stationId:stationId,isPolice:true,token:token})
        return;
    }
    console.log(req.body.email)
    ...
    res.status(200).send({message:"Authentication Successful",isPolice:user.isPolice,token:token,userId:user.id})
    return;
}
...
exports.getFir = async (req,res,next) => {
    try{
        ...
        let userFir = station.requests.filter(request=>request.id === req.body.userId)
